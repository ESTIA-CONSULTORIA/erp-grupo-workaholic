...
export class BulkImportService {
  ...
  async importGastos(companyId: string, rows: any[]) {
    ...
    for (const row of rows) {
      try {
        if (!row.concept) { errors.push(`Sin concepto: ${JSON.stringify(row)}`); continue; }
  ...
  async importProveedores(companyId: string, rows: any[]) {
    ...
    for (const row of rows) {
      try {
        if (!row.nombre && !row.name) { errors.push('Sin nombre'); continue; }
  ...
  async importProductos(companyId: string, rows: any[]) {
    let ok = 0; const errors: string[] = [];
    for (const row of rows) {
      try {
        if (!row.sku || !row.nombre) { errors.push(`SKU y nombre requeridos`); continue; }
        const exists = await this.prisma.product.findFirst({ where: { companyId, sku: row.sku } });
        if (exists) { errors.push(`SKU ya existe: ${row.sku}`); continue; }
        ...
        ok++;
      } catch(e: any) { errors.push(`Error ${row.sku}: ${e.message}`); }
    }
    return this.result(ok, errors);
  }

  async importInsumos(companyId: string, rows: any[]) {
    let ok = 0; const errors: string[] = [];
    for (const row of rows) {
      try {
        if (!row.nombre && !row.name) { errors.push('Sin nombre'); continue; }
        const name = row.nombre || row.name;
        const exists = await (this.prisma as any).insumo.findFirst({ where: { companyId, name } });
        if (exists) { errors.push(`Ya existe: ${name}`); continue; }
        await (this.prisma as any).insumo.create({ data: {
          companyId,
  ...
  async importEmpleados(companyId: string, rows: any[]) {
    let ok = 0; const errors: string[] = [];
    ...
    for (const row of rows) {
      try {
        if (!row.nombre && !row.firstName) { errors.push('Sin nombre'); continue; }
  ...
  async importCxC(companyId: string, rows: any[]) {
    ...
    for (const row of rows) {
      try {
        if (!row.cliente && !row.clientId) { errors.push('Sin cliente'); continue; }
        let clientId = row.clientId;
        if (!clientId) {
          const cl = await this.prisma.client.findFirst({ where: { companyId, name: { contains: row.cliente } } });
          if (!cl) { errors.push(`Cliente no encontrado: ${row.cliente}`); continue; }
          clientId = cl.id;
        }
  ...
  async importCxP(companyId: string, rows: any[]) {
    ...
    for (const row of rows) {
      try {
        if (!row.proveedor && !row.supplierId) { errors.push('Sin proveedor'); continue; }
        let supplierId = row.supplierId;
        if (!supplierId) {
          const sp = await this.prisma.supplier.findFirst({ where: { companyId, name: { contains: row.proveedor } } });
          if (!sp) { errors.push(`Proveedor no encontrado: ${row.proveedor}`); continue; }
          supplierId = sp.id;
        }
        const amount = Number(row.monto || row.originalAmount || 0);
